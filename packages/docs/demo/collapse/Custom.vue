...
import type { CollapseItemName } from "lotus-plus";
import { ref } from "vue";

const activeNames = ref(["1"]);

const handleClick = (names: CollapseItemName[]) => {
  console.log(names);
};
</script>
...
<template>
  <div>
    <l-collapse v-model="activeNames" @change="handleClick">
      <l-collapse-item title="Consistency" name="1">
        <template #title>
          <span style="color: var(--danger-color)"> Consistency </span>
          <l-icon icon="question-circle" />
        </template>
        <div>
